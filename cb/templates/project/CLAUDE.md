...
Driven from Claude Code. `cb` is deterministic Python and never calls an LLM —
the judgement is mine, and it lives in `skills/`. Read the relevant skill before
acting; they are prose and they are meant to be edited.

## Where things are

- `wiki/` — what is known about this company: tables, the causal graph (one file
  per node), business rules, process, experiments, traps.
